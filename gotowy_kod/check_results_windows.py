...
def run(popenargs):
    with subprocess.Popen(popenargs, stdout=subprocess.PIPE, shell=False) as process:
        try:
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(os.path.basename(sys.argv[0]) + " <katalog_z_programem_run.bat> <sciezka_do_katalogu_ze_zbiorami_danych>")
        exit(1)
